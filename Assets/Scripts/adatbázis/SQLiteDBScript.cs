    ...
    public int InsertPlayerData(int playerAge, string generation, int simonScore, double mazeTime, int shootingScore)
    {
        ...
        try
        {
            using (IDbConnection dbConnection = new SqliteConnection(connectionString))
            {
                ...
                string dateStr = DateTime.Now.ToString("yyMMdd");  // Csak dátum: év, hónap, nap (6 számjegy)
                string timeStr = DateTime.Now.ToString("HHmm");    // Óra és perc (4 számjegy)
                string idStr = dateStr + timeStr.Substring(0, Math.Min(2, timeStr.Length));  // Csak az órát vesszük (8 számjegy összesen)
